...
data$feed <- as.numeric(as.character(data$feed))
...
optim(c(beta_0 = -1,beta_1= -0.7,beta_2= 0.5,beta_3= 1,rho = 0.2),val,method ="L-BFGS-B", lower = c(rep(-Inf,4),0), upper = c(rep(10,4),1))
...
### The following IVs have lead to convergence = 0:
optim(c(beta_0 = -1,beta_1=-0.7,beta_2= 0.1,beta_3= 1,rho = 0.2),val1, method = "L-BFGS-B", lower = c(rep(-Inf,4),0), upper = c(rep(10,4),1) )


## Obtain the asymptotic properties:
...
variance_3 <- function(data,alpha,beta_0,beta_1,beta_2,beta_3,rho){
  ...
  omega_k <- Reduce('+',t2)
  cov_matrix <- ginv(psi_k) %*% omega_k %*% ginv(psi_k)
  cov_matrix
}


variance_3(data_1,0.3,-3.452,-1.818,3.431,-1.452,0.171) ## Final form
